...
void CSpO2Info::ShowSpO2Info(CPaintDC* pdc)
{
	...
	else strSpO2.Format(_T("%d"), m_uiSpO2); kirimFloat(m_uiSpO2, "m_uiSpO2.txt");

	if(m_uiPulseRate == INVALID_PR_VALUE)
	{
		strPr.Format(_T("--"));
	}
	else strPr.Format(_T("%d"), m_uiPulseRate); kirimFloat(m_uiPulseRate, "m_uiPulseRate.txt");

	if(m_uiPi == INVALID_PI_VALUE)
	{
		strPi.Format(_T("-.--"));
	}
	else strPi.Format(_T("%.2f"), m_uiPi / 1000.0); kirimFloat(m_uiPi / 1000.0, "m_uiPi.txt");
